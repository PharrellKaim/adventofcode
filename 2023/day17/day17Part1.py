...
def dijkstra(graph: list[list[int]], max_steps: int):
    ...
    while queue:
        ...
        for new_direction in Direction:
            ...
            if new_direction.value[0] + curr.direction.value[0] == 0 and new_direction.value[1] + curr.direction.value[
                1] == 0:
                continue
...
def main():
    with open(os.path.join(sys.path[0], "inputDay17"), "r", encoding="utf-8") as f:
        text = f.read().strip()
        lines = text.split("\n")
